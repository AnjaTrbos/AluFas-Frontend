...
const stepTitleStyle = { fontSize: 'clamp(1rem, 3vw, 1.35rem)', fontWeight: 900, color: '#0f172a', textAlign: 'left' } as const;
const expandedPanelStyle = { marginTop: '0.55rem', borderRadius: '1rem', border: '1.5px solid #d8e1ec', background: '#ffffff', padding: '1rem 1rem 1.05rem' } as const;
const detailTextStyle = { margin: '0 0 0.5rem', fontSize: 'clamp(0.95rem, 2.8vw, 1.05rem)', fontWeight: 700, color: '#0f172a' } as const;
const detailTextLastStyle = { margin: '0 0 1rem', fontSize: 'clamp(0.95rem, 2.8vw, 1.05rem)', fontWeight: 700, color: '#0f172a' } as const;
const detailLabelStyle = { fontWeight: 900 } as const;
const resultHeadingStyle = { margin: '0 0 0.65rem', fontSize: 'clamp(1.1rem, 3vw, 1.35rem)', fontWeight: 900, color: '#0f172a' } as const;
...
const resultTextStyle = { fontSize: 'clamp(1rem, 2.8vw, 1.2rem)', fontWeight: 700, color: '#0f172a' } as const;
									...
									{resultOptions.map((option) => {
										...
										return (
											<button key={option} type="button" onClick={() => onToggleResult(step.id, option)} style={resultButtonStyle}>
												<span style={{ ...resultIndicatorStyle, border: checked ? '2px solid #1e3a8a' : '2px solid #b7c4d6', background: checked ? '#1e3a8a' : '#ffffff' }}>
